...
DOCKER_ENV=("-e" "TASK_ID=124" "-e" "REDIS_URL=redis://redis:6379/0" "-e" "REDIS_CHANNEL=task_updates")
...
if [[ $DRYRUN -eq 1 ]]; then
	...
	echo
	echo docker "${DOCKER_OPTS[@]}" "${DOCKER_MOUNTS[@]}" "${DOCKER_ENV[@]}" "$ANALYSIS_IMAGE" "${ANALYSIS_ARGS[@]}"
